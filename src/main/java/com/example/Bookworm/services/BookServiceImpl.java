...
import com.example.Bookworm.repositories.BookRepository;
...
public class BookServiceImpl implements BookService {
	...
	public ResponseEntity<?> toggleStatus(Long id) {
		...
		if(optionalExistingBU.isPresent()) {
			...
			return ResponseEntity.status(HttpStatus.ACCEPTED).body(new ResponseMessage("Status changed"));
		}
		
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseMessage("Status has not changed"));
	}
